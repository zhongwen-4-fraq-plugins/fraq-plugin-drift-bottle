...
export function registerHelpCommand(ctx: Context): void {
  async function showHelp(session: Session): Promise<void> {
    const commands = [
      ...
      ['漂流瓶审核 通过 <审核记录ID>', '通过待审核投瓶并公开，仅插件主人和漂流瓶管理员可用。'],
      ['漂流瓶审核 拒绝 <审核记录ID> <理由>', '拒绝待审核投瓶并归档，拒绝理由必填。'],
      ['漂流瓶权限 添加 <QQ号或@用户...>', '添加一个或多个管理用户，可删除和审核漂流瓶，仅插件主人可用。'],
      ['漂流瓶权限 删除 <QQ号或@用户...>', '移除一个或多个漂流瓶管理用户，仅插件主人可用。'],
      ['漂流瓶权限 列表', '查看漂流瓶管理权限列表，仅插件主人可用。'],
