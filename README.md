...
插件首次载入时会为 `ownerIds` 中的第一个 QQ 号创建 WebUI 账号，并生成一个 10 位、同时包含大写字母、小写字母和数字的初始密码。账号和密码只在生成当次写入日志，SQLite 仅保存带盐密码哈希；旧版单密码会自动迁移到首位主人账号。

其他 QQ 号可以在登录页设置 6–10 位、同时包含大小写英文字母和数字的密码并提交注册申请。机器人会私聊所有主人；任一主人发送以下命令即可同意：
