...
The persisted forwarded-message structure is the source of truth. Return each top-level sender as a separate text field and its body as Markdown so React can render the trusted structure without encoding names into Markdown. Generated metadata inside nested Markdown must still be Markdown-escaped. Message text and native Markdown segments may retain Markdown syntax intentionally.
...
When the list previews only the first few top-level messages, return the structured message rows and total count once, then slice the array in the renderer. Do not split or join rendered Markdown with horizontal-rule text because nested forwards and user-authored Markdown can contain the same delimiter. Desktop and mobile views should share the same expansion key for each record and segment.
