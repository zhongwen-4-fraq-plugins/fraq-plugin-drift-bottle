...
WebUI 全部瓶子列表的新增、修改、删除及混合消息文字编辑已随 v0.3.23 发布；真实 Fraq 宿主完成安装、独立端口启动和未登录写接口回归。
...
- 用户方便时重启原有 `4649` 宿主，使既有运行实例加载磁盘上的 `0.3.23`。
...
- GitHub Publish 工作流、GitHub Release 和 npm `0.3.23` 发布成功，镜像元数据包含完整性与 SLSA provenance。
- `D:\bot\fraq-plugins\my-fraq-app` 的 `versions.yml`、生成清单和已安装包均更新为 `0.3.23`。
- 使用临时 `4650` 与独立 SQLite 启动 `0.3.23`；WebUI 页面、会话和构建资源均返回 `200`，未登录修改返回 `401`。
- `0.3.23` 验证进程链、端口、日志和独立数据库均已清理；原有 `4649` PID `36584` 保持运行。
