...
WebUI 合并转发默认以“名字：消息”显示前四条，不生成标题或消息分割线；点击后显示全部并可收起，已完成本地构建、测试及桌面/窄屏视觉验证。
...
- 合并转发发送者和正文由结构化消息生成，正文保留 Markdown，并支持两层嵌套转发。
- Markdown 渲染禁用原始 HTML、限制链接协议，并将外部图片替换为文字占位。
- GFM 表格和代码块使用局部横向滚动，普通正文在窄栏中保持换行；无消息明细时保留原摘要。
- 合并转发按顶层消息返回稳定序号、发送者、Markdown 正文与总条数，前端直接截取前四条，避免重复序列化或按分隔线切割。
- 超过四条时显示“查看全部”入口，展开后可收起；桌面与移动视图共享每个瓶子消息段的展开状态。
- 顶层转发以“名字：消息”自然行内排版，不显示转发标题或自动分割线；表格、代码和嵌套消息仍保留受限 Markdown。

Current:

- 合并转发四条“名字：消息”预览已完成本地实现与验证；等待发布并登录真实宿主回归。
...
- `pnpm check`、`pnpm test`（47 项）与 `pnpm build` 通过；Impeccable 布局及排版机械扫描均为 0 项。
- Chrome 桌面默认显示 4 条无分割线消息；288px 窄栏展开显示 7 条，`clientWidth` 与 `scrollWidth` 均为 288px，自动 `hr` 数量为 0。
